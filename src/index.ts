...
export default {
    ...
    scheduled: async (event: any, env: any, ctx: any) => {
        ctx.waitUntil((async () => {
            ...
            try {
                const req = new Request('http://localhost/api/prices/anekalogam');
                ...
                if (res.ok) {
                    ...
                    if (item) {
                        const text = `📊 *Update Harga Emas Antam (Aneka Logam)*\n\n` +
                ...
                console.error("Gagal menjalankan cron job Telegram.");
            }
        })());
    }
};
